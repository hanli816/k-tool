...
export const useTaskSync = () => {
    ...
    try {
      ...
      if (savedData) {
        ...
        return { 
          ...
          systemLogo: parsed.systemLogo || undefined
        };
      }
    } catch (e) {}
  ...
  const fetchSystemConfig = useCallback(async () => {
    ...
    try {
      const response = await fetch(`${state.cloudConfig?.supabaseUrl}/rest/v1/user_data?id=eq.${SYSTEM_CONFIG_ID}&t=${timestamp}`, {
        method: 'GET',
        headers: {
          'apikey': state.cloudConfig?.supabaseKey || '',
          'Authorization': `Bearer ${state.cloudConfig?.supabaseKey}`, // 使用 anon key
        }
      });
      if (response.ok) {
        const result = await response.json();
        if (result && result.length > 0) {
          const cloudLogo = result[0].data?.customLogo;
          if (cloudLogo && cloudLogo !== state.systemLogo) {
            console.log('Global logo synced from cloud');
            setState(prev => ({ ...prev, systemLogo: cloudLogo }));
          }
        }
      }
    } catch (err) {
  ...
  // 定时每 10 分钟自动拉取一次全局配置（或者当登录状态改变时）
  useEffect(() => {
    if (state.user) {
      fetchSystemConfig();
    }
    ...
    Object.keys(state.lists).forEach(dateKey => {
      if (dateKey < todayKey) {
        ...
        if (unfinished.length > 0) {
          ...
          updatedLists[dateKey] = { ...updatedLists[dateKey], tasks: taskIds.filter(id => !unfinished.includes(id)) };
          unfinished.forEach(id => { updatedTasks[id] = { ...updatedTasks[id], listDate: todayKey }; });
        }
      }
    });
    if (changed) {
      const todayList = updatedLists[todayKey] || { date: todayKey, tasks: [] };
      updatedLists[todayKey] = { ...todayList, tasks: [...unfinishedToMigrate, ...todayList.tasks] };
      setState(prev => ({ ...prev, tasks: updatedTasks, lists: updatedLists }));
    }
  ...
  useEffect(() => {
    ...
    const performSync = async () => {
      if (!hasInitialFetched.current) {
        ...
        try {
          const response = await fetch(`${supabaseUrl}/rest/v1/user_data?id=eq.${userId}`, { method: 'GET', headers });
          if (response.ok) {
            const result = await response.json();
            if (result && result.length > 0) {
      ...
      const pushToCloud = async () => {
        ...
        try {
          ...
          // 如果是管理员，同步写入全局记录行
          if (isAdmin) {
             console.log('Admin detected, pushing global config...');
             promises.push(
               fetch(`${supabaseUrl}/rest/v1/user_data`, {
                method: 'POST',
                headers: { ...headers, 'Prefer': 'resolution=merge-duplicates' },
                body: JSON.stringify({
                  id: SYSTEM_CONFIG_ID,
                  data: { customLogo: state.customLogo }
                })
              })
             );
          }
          ...
          if (results.every(r => r.ok)) {
            ...
            if (isAdmin) fetchSystemConfig(); // 管理员自己也刷一下
          } else {
             const errorTexts = await Promise.all(results.map(r => r.text()));
             console.error('Sync error texts:', errorTexts);
             setState(prev => ({ ...prev, syncStatus: 'error' }));
          }
        } catch (err) {
  ...
  const addDoc = (parentId: string | null = null, initial?: Partial<Document>) => {
    ...
    const newDoc: Document = { id, title: initial?.title || '', content: initial?.content || '', updatedAt: Date.now(), isPinned: false, images: [], parentId: parentId, order: Date.now(), linkedTaskIds: initial?.linkedTaskIds || [] };
    setState(prev => ({ ...prev, docs: { ...prev.docs, [id]: newDoc }, activeDocId: id }));
    return id;
  };
  const updateDoc = (id: string, updates: Partial<Document>) => setState(prev => {
    const doc = prev.docs[id]; if (!doc) return prev;
    return { ...prev, docs: { ...prev.docs, [id]: { ...doc, ...updates, updatedAt: Date.now() } } };
  });
  const deleteDoc = (id: string) => setState(prev => {
    const newDocs = { ...prev.docs }; (Object.values(newDocs) as Document[]).forEach(d => { if (d.parentId === id) d.parentId = null; });
    delete newDocs[id]; return { ...prev, docs: newDocs, activeDocId: prev.activeDocId === id ? null : prev.activeDocId };
  });
  const moveDoc = (docId: string, newParentId: string | null, newOrder: number) => setState(prev => {
    const doc = prev.docs[docId]; if (!doc) return prev;
  ...
  const linkTaskToDoc = (taskId: string, docId: string) => setState(prev => {
    const task = prev.tasks[taskId]; const doc = prev.docs[docId]; if (!task || !doc) return prev;
    const taskDocs = new Set(task.linkedDocIds || []); const docTasks = new Set(doc.linkedTaskIds || []);
    taskDocs.add(docId); docTasks.add(taskId);
    return { ...prev, tasks: { ...prev.tasks, [taskId]: { ...task, linkedDocIds: Array.from(taskDocs) } }, docs: { ...prev.docs, [docId]: { ...doc, linkedTaskIds: Array.from(docTasks) } } };
  });

  const saveSolutionToKB = async (taskId: string, targetDocId?: string) => {
    const task = state.tasks[taskId]; if (!task || !task.aiSolution) return;
    if (targetDocId) { const doc = state.docs[targetDocId]; if (doc) { const newContent = doc.content + `\n\n---\n### AI Solution for Task: ${task.name}\n` + task.aiSolution; updateDoc(targetDocId, { content: newContent }); linkTaskToDoc(taskId, targetDocId); } }
    else { const newDocId = addDoc(null, { title: `Solution: ${task.name}`, content: `# AI Solution for ${task.name}\n\n${task.aiSolution}`, linkedTaskIds: [taskId] }); linkTaskToDoc(taskId, newDocId); }
  };
  ...
  const updateTask = (id: string, updates: Partial<Task>) => setState(prev => {
    const task = prev.tasks[id]; if (!task) return prev;
    let completedAt = task.completedAt; if (updates.status === TaskStatus.DONE && task.status !== TaskStatus.DONE) completedAt = getNowTimestamp();
  ...
  const deleteTask = (id: string) => setState(prev => {
    ...
    const newLists = { ...prev.lists }; Object.keys(newLists).forEach(d => { if(newLists[d]) newLists[d].tasks = newLists[d].tasks.filter(tid => tid !== id); });
